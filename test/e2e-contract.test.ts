...
test('RequestMap is exhaustively listed and every kind has hostile/error contract coverage', async (t) => {
  assert.equal(new Set(kinds).size, 26);
  const repo = await makeRepo({ label: 'contract' });
  const h = repo.harness({ host: { iconThemeSnapshot: async () => null, openDiff: async () => ({ opened: true, mode: 'worktree' }), openExplorer: () => {}, revealCommit: async () => true, openExternal: async () => true, githubRepo: async () => ({ defaultBranch: 'main', private: false, htmlUrl: 'https://github.com/x/y', rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubPullRequests: async () => ({ pullRequests: [], rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubCommitAuthors: async () => ({ authors: [], rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubContributorIdentity: async () => ({ login: 'octocat', avatarUrl: 'https://avatars.githubusercontent.com/u/1', htmlUrl: 'https://github.com/octocat' }), githubLinkage: async () => ({ available: false, host: null, owner: null, repo: null, webUrl: null, commitUrlTemplate: null, apiUrl: null }) } }); own(t, repo, h);
  ...
  for (const kind of kinds) {
    ...
    const absent = repo.harness({ host: { resolveRepository: async () => null, openDiff: async () => ({ opened: true, mode: 'worktree' }), openExplorer: () => {}, revealCommit: async () => true, githubRepo: async () => ({ defaultBranch: 'main', private: false, htmlUrl: '', rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubPullRequests: async () => ({ pullRequests: [], rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubCommitAuthors: async () => ({ authors: [], rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubContributorIdentity: async () => ({ login: null, avatarUrl: null, htmlUrl: null }), githubLinkage: async () => ({ available: false, host: null, owner: null, repo: null, webUrl: null, commitUrlTemplate: null, apiUrl: null }) } });
    const missing = await absent.send(kind, valid(kind, head)); checkResponse(missing); absent.dispose();
    if (!missing.ok && ['repos/status','repos/graph','repos/remotes','repos/contributors','stash/list','stash/show','commits/detail','actions/stage','actions/commit','actions/git','actions/openDiff'].includes(kind)) assert.equal(missing.error.code, 'NOT_FOUND');
  }
});
