...
const kinds: readonly RequestKind[] = ['repos/status','repos/graph','repos/remotes','repos/contributors','stash/list','stash/show','commits/detail','actions/stage','actions/commit','actions/git','actions/openDiff','actions/openStashDiff','actions/showLogs','actions/openExplorer','graph/revealCommit','actions/openExternal','github/auth','github/connect','github/disconnect','github/repo','github/pullRequests','github/commitAuthors','github/contributorIdentity','github/linkage','settings/get','settings/set','iconTheme/get'];
...
function valid(kind: RequestKind, head: string): Record<string, unknown> {
  switch (kind) {
    ...
    case 'graph/revealCommit': return { hash: head };
    case 'stash/show': return { index: 0 };
    ...
    case 'actions/openStashDiff': return { index: 0, path: 'c1.txt' };
    ...
    case 'github/contributorIdentity': return { email: 'contributor@example.com' };
...
function hostile(kind: RequestKind): Record<string, unknown> {
  ...
  if (kind === 'repos/remotes' || kind === 'repos/contributors' || kind === 'stash/list' || kind === 'actions/showLogs' || kind === 'actions/openExplorer' || kind === 'github/auth' || kind === 'github/connect' || kind === 'github/disconnect' || kind === 'github/linkage') return { value: null };
  if (kind === 'stash/show') return { index: -1 };
  if (kind === 'commits/detail' || kind === 'graph/revealCommit') return { hash: '--force' };
  ...
  if (kind === 'actions/openStashDiff') return { path: '../../etc/passwd', index: -1 };
  ...
  if (kind === 'github/contributorIdentity') return { email: 'x'.repeat(1_000_000) };
...
test('RequestMap is exhaustively listed and every kind has hostile/error contract coverage', async (t) => {
  assert.equal(new Set(kinds).size, 27);
  const repo = await makeRepo({ label: 'contract' });
  const h = repo.harness({ host: { iconThemeSnapshot: async () => null, openDiff: async () => ({ opened: true, mode: 'worktree' }), openStashDiff: async () => ({ opened: true, mode: 'stash' }), openExplorer: () => {}, revealCommit: async () => true, openExternal: async () => true, githubRepo: async () => ({ defaultBranch: 'main', private: false, htmlUrl: 'https://github.com/x/y', rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubPullRequests: async () => ({ pullRequests: [], rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubCommitAuthors: async () => ({ authors: [], rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubContributorIdentity: async () => ({ login: 'octocat', avatarUrl: 'https://avatars.githubusercontent.com/u/1', htmlUrl: 'https://github.com/octocat' }), githubLinkage: async () => ({ available: false, host: null, owner: null, repo: null, webUrl: null, commitUrlTemplate: null, apiUrl: null }) } }); own(t, repo, h);
  ...
  for (const kind of kinds) {
    ...
    const absent = repo.harness({ host: { resolveRepository: async () => null, openDiff: async () => ({ opened: true, mode: 'worktree' }), openStashDiff: async () => ({ opened: true, mode: 'stash' }), openExplorer: () => {}, revealCommit: async () => true, githubRepo: async () => ({ defaultBranch: 'main', private: false, htmlUrl: '', rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubPullRequests: async () => ({ pullRequests: [], rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubCommitAuthors: async () => ({ authors: [], rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubContributorIdentity: async () => ({ login: null, avatarUrl: null, htmlUrl: null }), githubLinkage: async () => ({ available: false, host: null, owner: null, repo: null, webUrl: null, commitUrlTemplate: null, apiUrl: null }) } });
    const missing = await absent.send(kind, valid(kind, head)); checkResponse(missing); absent.dispose();
    if (!missing.ok && ['repos/status','repos/graph','repos/remotes','repos/contributors','stash/list','stash/show','commits/detail','actions/stage','actions/commit','actions/git','actions/openDiff','actions/openStashDiff'].includes(kind)) assert.equal(missing.error.code, 'NOT_FOUND');
  }
});
