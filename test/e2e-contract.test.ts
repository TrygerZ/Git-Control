...
test('RequestMap is exhaustively listed and every kind has hostile/error contract coverage', async (t) => {
  assert.equal(new Set(kinds).size, 22);
  ...
  for (const kind of kinds) {
    ...
    const absent = repo.harness({ host: { resolveRepository: async () => null, openDiff: async () => ({ opened: true, mode: 'worktree' }), openExplorer: () => {}, githubRepo: async () => ({ defaultBranch: 'main', private: false, htmlUrl: '', rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubPullRequests: async () => ({ pullRequests: [], rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubCommitAuthors: async () => ({ authors: [], rateLimit: { limit: null, remaining: null, resetAt: null, cached: false, offline: false } }), githubLinkage: async () => ({ available: false, host: null, owner: null, repo: null, webUrl: null, commitUrlTemplate: null, apiUrl: null }) } });
