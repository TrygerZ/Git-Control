...
function hostile(kind: RequestKind): Record<string, unknown> {
  ...
  if (kind === 'repos/remotes' || kind === 'repos/contributors' || kind === 'actions/showLogs' || kind === 'actions/openExplorer' || kind === 'github/auth' || kind === 'github/connect' || kind === 'github/disconnect' || kind === 'github/linkage') return { value: null };
...
test('RequestMap is exhaustively listed and every kind has hostile/error contract coverage', async (t) => {
  assert.equal(new Set(kinds).size, 23);
  ...
  for (const kind of kinds) {
    ...
    if (!missing.ok && ['repos/status','repos/graph','repos/remotes','repos/contributors','commits/detail','actions/stage','actions/commit','actions/git','actions/openDiff'].includes(kind)) assert.equal(missing.error.code, 'NOT_FOUND');
  }
});
